...
class UnivariateLaplaceNoiseModule(NoiseModule[IndependentNoise[UnivariateLaplaceNoise]]):
    ...
    def forward(self, x: Optional[torch.Tensor] = None) -> IndependentNoise[UnivariateLaplaceNoise]:
        if x is None:
            x = torch.zeros_like(self.log_scale)
        return IndependentNoise(UnivariateLaplaceNoise(loc=x, scale=torch.exp(self.log_scale)), 1)
