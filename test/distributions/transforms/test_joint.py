...
from causica.distributions.transforms.joint import JointTransform
...
def test_joint_transform_call_and_inv(affine_transform):
    ...
    assert torch.allclose(transformed_data["x"], data.get("x"), atol=1e-6)
    assert torch.allclose(transformed_data["y"], data.get("y") * 2 + 1, atol=1e-6)

    assert torch.allclose(affine_transform.inv(transformed_data)["x"], data.get("x"), atol=1e-6)
    assert torch.allclose(affine_transform.inv(transformed_data)["y"], data.get("y"), atol=1e-6)
...
def test_joint_transform_passthrough(affine_transform):
    data = TensorDict(
        {"x": torch.randn((100, 1)), "y": torch.randn((100, 1)), "z": torch.randn((100, 3))}, batch_size=100
    )
    assert torch.allclose(affine_transform(data)["z"], data.get("z"))
