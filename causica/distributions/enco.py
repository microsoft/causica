...
from .utils import fill_triangular, gumbel_softmax_binary
...
class ENCOAdjacencyDistribution(AdjacencyDistribution):
    ...
    def entropy(self) -> torch.Tensor:
        ...
        torch.diagonal(entropy, dim1=-2, dim2=-1).zero_()  # zero the diagonal elements
    ...
    def mean(self) -> torch.Tensor:
        ...
        """
        logits = self._get_independent_bernoulli_logits()
        mean = self.base_dist(logits).mean
        torch.diagonal(mean, dim1=-2, dim2=-1).zero_()  # zero the diagonal elements
        return mean
    ...
    def mode(self) -> torch.Tensor:
        ...
        """
        logits = self._get_independent_bernoulli_logits()
        mode = self.base_dist(logits).mode
        torch.diagonal(mode, dim1=-2, dim2=-1).zero_()  # zero the diagonal elements
        return mode
